    ...
    public class MusicReleasesController : Controller
    {
        ...
        public ActionResult Index(string search, string filter, int? i, string date)
        {
            ...
            if(date != null)
            {
                ...
                {
                    //finalList = list.OrderByDescending(o => o.Sales_Start_Date).ToList();
            ...
            else
            {
                ...
                //var newList = new List<MusicRelease>();
                //for (int j = finalList.Count - 1; j > -1; j--)
                //{
                //    newList.Add(finalList[j]);
                //}
        ...
        List<SpotifyReports_2020> SpotifyReport(string upc)
        {
            ...
            foreach (var x in yearsMonths)
            {
                ...
                obj.USD_Payable = (decimal)payable;
        ...
        public ActionResult Create([Bind(Include = "MusicReleaseId,ReleaseName,MainArtist,ReleaseVersion,RecordingLocation,Language,CopyrightYear,ReleaseDate,OriginalReleaseDate,UPCEAN,LabelName,CInfo,CYear,PInfo,PYear,CopyryghtHolder,Publisher")] MusicRelease musicRelease,
            string ReleaseFormat, string Advisory, string Category, string Territory, string stripeEmail,string IS_VAR, string[] VAR, string [] composer_song, string[] writer_song, string [] arranger_song, string [] iswc_song, string paymentMethod, string priceCharge, string stripeToken, 
            string[] FeaturedArtist, string[] Title, string[] ISrc, string Explicit, string [] PartName_New, string [] PartRole_New, string[] PartType_New, string[] PartInstrument_New, string[] PartIPICode_New, string[] PartIPNCode_New,
            string[] Store, HttpPostedFileBase coverImage, string country, string needUPC, string [] artist_song, string [] trackversion_song, string [] cinfo_song, string [] cyear_song, string [] pinfo_song, string [] pyear_song, string [] copyright_song, string [] participent_song)
        {
            ...
            //{
                ...
                foreach (var x in ISRC)
                {
                    if (x.ISRC.Equals(musicRelease.UPCEAN))
                        x.ISRC_status = "Used";
                }
                db.SaveChanges();
                ...
                //musicRelease.MainArtist = String.Join("-", Title);
                if (needUPC == null)
                {
                ...
                var sd = Request.Form["Compilation"];
                ...
                if (Title.Length > 0)
                {
                    ...
                    //content.AppendLine("Check No.,Grouping ID,Product Title,Version Description,Artist(s),Display Artist,Barcode,Catalog Number,Release Format Type,Sound Carrier," +
                    //    "Price Band,Licensed Territories to Include,Licensed Territories to Exclude,Release Start Data,Release End Date,Grid,(P)Year,(P)Holder,(C)Year,(C)Holder," +
                     //   "Status,Label,MainGenre Primary,MainGenre Sub,AlternativeGenre Primary,AlternativeGenre Sub,Explicit Content,Volume Number,Volume Total,Services," +
                     //   "Track Number,Track Title,Mix/Version,Artist(S),Display Artist,ISRC,GRid,AVAILABLE SEPARATELY,(P)YEAR,(P)HOLDER,(C)YEAR,(C)HOLDER," +
                     //   "MainGenre Primary,MainGenre Sub,AlternativeGenre Primary,AlternativeGenre Sub,Explicit Content,PRODUCER(S),MIXER(S),COMPOSER(S),LYRICIST(S),PUBLISHER(S),WRITER(S),ARRANGER(S)");
                    ...
                    if (user.LabelName != "")
                    {
                        ServerSavePath = "musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN +"/" ;
                    }
                    else
                    {
                        ServerSavePath = "musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/";
                    }

                    var isrc = db.UPC_ISRCs.ToList();
                    foreach (var x in ISrc)
                    {
                        foreach (var y in isrc)
                        {
                            if (x.Equals(y.ISRC))
                                y.ISRC_status = "Used";
                        }
                    ...
                    foreach (var y in isrc)
                    {
                        if (musicRelease.UPCEAN.Equals(y.UPC))
                            y.UPC_status = "Used";
                    }
                    db.SaveChanges();
                    ...
                    for (int i =0; i < Title.Length; i++)
                    {
                        if (Title[i] != null)
                        {
                            //var InputFileName = Path.GetFileName(songs[i].FileName);
                            ...
                            if (sIndex < 10)
                            {
                                if(user.LabelName != "")
                                {
                                    song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "/" + musicRelease.UPCEAN + "_01_0" + sIndex + ".wav";
                                    //SongPath = ServerSavePath + musicRelease.UPCEAN + "_01_0" + sIndex + ".wav";
                                }
                                else
                                {
                                    song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "_01_0" + sIndex + ".wav";
                                    //SongPath = ServerSavePath + musicRelease.UPCEAN + "_01_0" + sIndex + ".wav";
                                }   
                            }
                            else
                            { 
                                ...
                                {
                                    song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "_01_" + sIndex + ".wav";
                                    //SongPath = ServerSavePath + musicRelease.UPCEAN + "_01_" + sIndex + ".wav";
                                }                      
                            }
                            ...
                            //song.ComposerName = composer_song[i];
                            //song.WriterName = writer_song[i];
                            //song.ArrangerName = arranger_song[i];
                            ...
                            ViewBag.SongsUploadStatus = Title.Count().ToString() + " files uploaded successfully.";
                            ...
                            //string line = "1,,"+musicRelease.ReleaseName+",,"+ musicRelease.MainArtist + ","+ FeaturedArtist + "," + upcean + ",,Album,," +
                            //    "Full,WORLD,,"+musicRelease.Sales_Start_Date+",,,"+musicRelease.CopyrightYear+","+musicRelease.CopyryghtHolder+ "," + musicRelease.CopyrightYear + "," + musicRelease.CopyryghtHolder + "," +
                            //    ","+musicRelease.LabelName+","+musicRelease.PrimaryGenre+",,"+musicRelease.SecondaryGenre+",,N,1,1,," +
                            //    ""+sIndex.ToString()+","+song.SongTitle+",,"+ musicRelease.MainArtist + "," + musicRelease.FeaturedArtist + ","+song.isrc.ToString()+ ",,Y," + musicRelease.CopyrightYear + "," + musicRelease.CopyryghtHolder + "," + musicRelease.CopyrightYear + "," + musicRelease.CopyryghtHolder + "" +
                            //    ","+ musicRelease.PrimaryGenre + "," + musicRelease.SecondaryGenre + "," + musicRelease.SecondaryGenre + ",N,"+ Explicit + ",," + Composer[i]+",,"+musicRelease.Publisher+","+Writer[i]+","+Arranger[i];
                    ...
                    string path = ServerSavePath + musicRelease.UPCEAN + ".csv";
                    byte[] data = new ASCIIEncoding().GetBytes(content.ToString());
                    MemoryStream stream = new MemoryStream(data);
                    //System.IO.File.AppendAllText(path, content.ToString());
                    CloudBlockBlob cloudBlockBlob1 = cloudBlobContainer.GetBlockBlobReference(path);
                    ...
                    path = ServerSavePath + "CI-Delivery Template.csv";

                    byte[] data1 = new ASCIIEncoding().GetBytes(template.ToString());
                    MemoryStream stream1 = new MemoryStream(data1);
                    //System.IO.File.AppendAllText(path, content.ToString());
                    CloudBlockBlob cloudBlockBlob2 = cloudBlobContainer.GetBlockBlobReference(path);
                    cloudBlockBlob2.Properties.ContentType = "application/vnd.ms-excel";

                    //await cloudBlockBlob.UploadFromStreamAsync(file.InputStream);
                    cloudBlockBlob2.UploadFromStream(stream1);

                    //System.IO.File.AppendAllText(path, template.ToString());
                ...
                if (coverImage != null)
                {
                    ...
                    //System.Drawing.Image sourceimage = System.Drawing.Image.FromStream(coverImage.InputStream);
                    //sourceimage.Save(ms, System.Drawing.Imaging.ImageFormat.Png);
                    //System.Drawing.Bitmap compressed = new System.Drawing.Bitmap(ms);
                    //ms.Close();

                    //using (var memoryStream = new MemoryStream())
                    //{

                    //    compressed.Save(memoryStream, System.Drawing.Imaging.ImageFormat.Jpeg);
                       

                    //    var ImagePath1 = ServerSavePath + musicRelease.UPCEAN + "_compressed_" + ".jpg";
                    //    //Save file to server folder  
                    //    //coverImage.SaveAs(ImagePath);

                    //    CloudBlockBlob cloudBlockBlob1 = cloudBlobContainer.GetBlockBlobReference(ImagePath1);
                    //    cloudBlockBlob1.Properties.ContentType = "image/jpeg";

                    //    //await cloudBlockBlob.UploadFromStreamAsync(file.InputStream);
                    //    cloudBlockBlob1.UploadFromStream(memoryStream);
                    ...
                    if (user.LabelName != "")
                    {
                        musicRelease.CoverImage = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "/" + musicRelease.UPCEAN + ".jpg";
                    }
                    else
                    {
                        musicRelease.CoverImage = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + ".jpg";
                    }


                    //assigning file uploaded status to ViewBag for showing message to user.  
                    ViewBag.ImageUploadStatus = Title.Count().ToString() + " files uploaded successfully.";
                    ...
                    {
                        string path = ServerSavePath + musicRelease.UPCEAN + ".xml";
                        var doc = Single(musicRelease);
                        CloudBlockBlob cloudBlockBlob1 = cloudBlobContainer.GetBlockBlobReference(path);
                        cloudBlockBlob1.Properties.ContentType = "application/xml";

                        byte[] data = new ASCIIEncoding().GetBytes(doc.OuterXml);
                        MemoryStream stream = new MemoryStream(data);
                        cloudBlockBlob1.UploadFromStream(stream);
                    ...
                    "Details are Here:  https://tpdigital.blob.core.windows.net/musicdistribution/" + ServerSavePath + musicRelease.UPCEAN + ".csv" + "\n" +
                    "\nThank you.\n\nSender,\nTempo Digital Team.";
        ...
        public XmlDocument Single(MusicRelease music)
        {
            ...
            Node.AppendChild(doc.CreateTextNode("add"));
            release.AppendChild(Node);
            ...
            Node.AppendChild(doc.CreateTextNode(music.ReleaseName.ToString()));
            release.AppendChild(Node);
            ...
            foreach (var x in participents)
            {
                ...
                insideNode.AppendChild(doc.CreateTextNode(x.ParticipantRole.ToString()));
                Node.AppendChild(insideNode);
                insideNode = doc.CreateElement("Name");
            ...
            foreach (var x in songs)
            {
                ...
                foreach (var y in song_part)
                {
                    ...
                    in_insideNode.AppendChild(doc.CreateTextNode(y.ParticipantRole.ToString()));
                    insideNode.AppendChild(in_insideNode);
                    in_insideNode = doc.CreateElement("name");
        ...
        public XmlDocument EP(MusicRelease music)
        {
            var songs = db.Songs.Where(a => a.MusicReleaseId == music.MusicReleaseId).ToList();
            var participents = db.Participants.Where(a => a.MusicReleaseId == music.MusicReleaseId).ToList();
            ...
            string[] Store, string[] composer_song, string[] writer_song, string[] arranger_song, string[] iswc_song, string[] artist_song, string[] trackversion_song, string[] cinfo_song, string[] cyear_song, string[] pinfo_song, string[] pyear_song, string[] copyright_song, string[] participent_song)
        {
            try
            {
                ...
                if (songs != null)
                {
                    ...
                    for (int i = 0; i < songs.Length; i++)
                    {
                        ...
                        if (existSong != null)
                        {
                            ...
                            //existSong.ComposerName = composer_song[i];
                            //existSong.WriterName = writer_song[i];
                            //existSong.ArrangerName = arranger_song[i];
                        ...
                        else
                        {
                            if (songs[i] != null)
                            {
                                ...
                                var SongPath = ServerSavePath + ".wav";
                                
                                var song = new Song();
                                //Here I can add +01 and then "_"
                                if (sIndex < 10)
                                {
                                    if (user.LabelName != "")
                                    {
                                        song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + existedRelease.currentDateTime.ToString("yyyy-MM-dd") + "/" + existedRelease.LabelName + "/" + existedRelease.UPCEAN + "/" + existedRelease.UPCEAN + rand.ToString() + "_01_0" + sIndex + ".wav";
                                        SongPath = ServerSavePath + existedRelease.UPCEAN + rand.ToString() + "_01_0" + sIndex + ".wav";
                                    }
                                    else
                                    {
                                        song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + existedRelease.currentDateTime.ToString("yyyy-MM-dd") + "/" + existedRelease.LabelName + "/" + existedRelease.UPCEAN + rand.ToString() + "_01_0" + sIndex + ".wav";
                                        SongPath = ServerSavePath + existedRelease.UPCEAN + rand.ToString() + "_01_0" + sIndex + ".wav";
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + existedRelease.currentDateTime.ToString("yyyy-MM-dd") + "/" + existedRelease.LabelName + "/" + existedRelease.UPCEAN + rand.ToString() + "_01_" + sIndex + ".wav";
                                        SongPath = ServerSavePath + existedRelease.UPCEAN + rand.ToString() + "_01_" + sIndex + ".wav";
                                    }
                                }

                                CloudBlockBlob cloudBlockBlob = cloudBlobContainer.GetBlockBlobReference(SongPath);
                                cloudBlockBlob.Properties.ContentType = songs[i].ContentType;

                                //await cloudBlockBlob.UploadFromStreamAsync(file.InputStream);
                                cloudBlockBlob.UploadFromStream(songs[i].InputStream);

                                //song.SongPath = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "_" + sIndex + ".wav"; ;
                                song.SongTitle = Title[i];
                                song.isrc = ISrc[i];
                                song.MusicReleaseId = musicRelease.MusicReleaseId;
                                //song.ComposerName = composer_song[i];
                ...
                if (coverImage != null)
                {
                    ...
                    //if (user.LabelName != "")
                    //{
                    //    existedRelease.CoverImage = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "/" + musicRelease.UPCEAN + ".jpg";
                    //}
                    //else
                    //{
                    //    existedRelease.CoverImage = "https://tpdigital.blob.core.windows.net/musicdistribution/musicdistribution/" + dateToday + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + ".jpg";
                    //}

                    db.SaveChanges();
                    //assigning file uploaded status to ViewBag for showing message to user.  
                    ViewBag.ImageUploadStatus = songs.Count().ToString() + " files uploaded successfully.";
                }
                ...
                message.Body = "Hi,\nRelease was successfully updated.\n" +
                    "\nID:" + musicRelease.MusicReleaseId.ToString() + "\n" +
                    "Details are Here:  " + toRemove1+img1+ "\n"+
                    "\nThank you.\n\nSender,\nTempo Digital Team.";
                smtp.Send(message);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            if (user.LabelName != "")
            {
                ServerSavePath = "musicdistribution/" + musicRelease.currentDateTime.ToString("yyyy-MM-dd") + "/" + musicRelease.LabelName + "/" + musicRelease.UPCEAN + "/";
            }
            else
            {
                ServerSavePath = "musicdistribution/" + musicRelease.currentDateTime.ToString("yyyy-MM-dd") + "/" + musicRelease.LabelName + "/";
            }
            ...
            var link = "https://tpdigital.blob.core.windows.net/musicdistribution/" + ServerSavePath + musicRelease.UPCEAN + ".csv";
            
                message1.Body = "Hi,<br>Release was successfully Deleted.<br>" +
                "Music Release ID:  " + id + "<br>" +
                "Details are <a href='"+ link + "'> Here</a>" +
                "<br><br>Thank you.<br><br>Sender,<br>Tempo Digital Team.";
